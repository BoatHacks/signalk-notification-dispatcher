...
## [Unreleased]
- Dev tool: `scripts/send-alert.sh` injects a `distress`/`urgency`/`safety` notification for an arbitrary vessel MMSI via the WebSocket delta stream, as a one-off or repeated at an interval - for exercising the plugin's rules without a real DSC/AIS distress relay. Adds `ws` as a devDependency (not runtime)
